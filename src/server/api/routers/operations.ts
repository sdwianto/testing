...
export const operationsRouter = router({
  ...
  // ========================================
  // EQUIPMENT DOCUMENTS MANAGEMENT
  // ========================================

  listEquipmentDocuments: protectedProcedure
    .input(z.object({
      equipmentId: z.string(),
    }))
    .query(async ({ ctx, input }) => {
      const documents = await (ctx.prisma as any).equipmentDocument.findMany({
        where: {
          tenantId: ctx.tenantId,
          equipmentId: input.equipmentId,
          isActive: true,
  ...
  createEquipmentDocument: idempotentProcedure
    .input(z.object({
      equipmentId: z.string().min(1),
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const document = await (ctx.prisma as any).equipmentDocument.create({
        data: {
          ...data,
          tenantId: ctx.tenantId,
        },
  ...
  updateEquipmentDocument: idempotentProcedure
    .input(z.object({
      id: z.string(),
      documentType: z.string().min(1),
      documentName: z.string().min(1),
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const document = await (ctx.prisma as any).equipmentDocument.update({
        where: {
          id,
          tenantId: ctx.tenantId,
        },
  ...
  deleteEquipmentDocument: idempotentProcedure
    .input(z.object({
      id: z.string(),
      idempotencyKey: z.string().uuid(),
    }))
    .mutation(async ({ ctx, input }) => {
      ...
      await (ctx.prisma as any).equipmentDocument.update({
        where: {
          id,
          tenantId: ctx.tenantId,
        },
